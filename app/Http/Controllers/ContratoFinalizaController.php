...
class ContratoFinalizaController extends Controller
{
    

  protected $nombre_modulo = "Talento Humano";

    public function All_ContratoFinaliza(Request $request)
    {
        //
        ...
        if($request["busquedad"]){
            $ContratoFinalizas = CambioAfp::where("fecha_desde",">=",$request["busquedad"])
            ->where("fecha_hasta","<=",$request["busquedad"])
            ->where('id_establecimiento', '=', $user->id_establecimiento)
            ->paginate(10);
        }else{
            $ContratoFinalizas = CambioAfp::where("id",">",0)
            ->where('id_establecimiento', '=', $user->id_establecimiento)
            ->paginate(10);
        }
       
         return view('thumano.ContratoFinalizas.home', array("ContratoFinalizas"=>$ContratoFinalizas,"title_menu"=>"",
            "title"=>"Contratos por Finalizar","user"=>$user,"Modulos"=>$modulos,
            "nombre_modulo"=>$this->nombre_modulo)); 
    }



    public function ContratoFinaliza_update(Request $request){
          ...
          $elemento1->save();
         return redirect('/All_ContratoFinaliza')->with('status', "Elemento Actualizado Correctamente");
     ...
     public function ContratoFinaliza_create(Request $request){
        ...
        return redirect('/All_ContratoFinaliza')->with('status', "Elemento Creado Correctamente");
    }




    public function formulario_ContratoFinaliza($id,$ruta)
    {
        //
        ...
        if($ruta=="actualizar"){
          $ruta ="ContratoFinaliza_update";
           $elemento =CambioAfp::find($id);
           $estilo="none";
        }else if($ruta=="crear"){
          $ruta ="ContratoFinaliza_create";
          ...
          $elemento1->idtipoContratoFinaliza ="";
          ...
          $elemento = $elemento1;
           ...
           $ruta ="All_ContratoFinaliza";
           $estilo="none";
           $elemento =CambioAfp::find($id);
        }

      return view('thumano.ContratoFinalizas.formulario', array('elemento' => $elemento,"id"=>$id,"ruta"=>$ruta,"user"=>$user,"Modulos"=>$modulos,
 ...
 public function delete_ContratoFinaliza($id)
    {
      CambioAfp::destroy($id);
      return redirect('/All_ContratoFinaliza')->with('status', "Elemento Eliminado Correctamente");
    }
