...
class EmpleadoVacacioneController extends Controller
{
   protected $nombre_modulo = "Talento Humano";

    public function All_EmpleadoVacacione(Request $request)
    {
        //
        ...
        if($request["busquedad"]){
            $EmpleadoVacaciones = CambioAfp::where("fecha_desde",">=",$request["busquedad"])
            ->where("fecha_hasta","<=",$request["busquedad"])
            ->where('id_establecimiento', '=', $user->id_establecimiento)
            ->paginate(10);
        }else{
            $EmpleadoVacaciones = CambioAfp::where("id",">",0)
            ->where('id_establecimiento', '=', $user->id_establecimiento)
            ->paginate(10);
        }
       
         return view('thumano.EmpleadoVacaciones.home', array("EmpleadoVacaciones"=>$EmpleadoVacaciones,"title_menu"=>"",
            "title"=>"Empleados sin Vacaciones","user"=>$user,"Modulos"=>$modulos,
            "nombre_modulo"=>$this->nombre_modulo)); 
    }



    public function EmpleadoVacacione_update(Request $request){
          ...
          $elemento1->save();
         return redirect('/All_EmpleadoVacacione')->with('status', "Elemento Actualizado Correctamente");
     ...
     public function EmpleadoVacacione_create(Request $request){
        ...
        return redirect('/All_EmpleadoVacacione')->with('status', "Elemento Creado Correctamente");
    }




    public function formulario_EmpleadoVacacione($id,$ruta)
    {
        //
        ...
        if($ruta=="actualizar"){
          $ruta ="EmpleadoVacacione_update";
           $elemento =CambioAfp::find($id);
           $estilo="none";
        }else if($ruta=="crear"){
          $ruta ="EmpleadoVacacione_create";
          ...
          $elemento1->idtipoEmpleadoVacacione ="";
          ...
          $elemento = $elemento1;
           ...
           $ruta ="All_EmpleadoVacacione";
           $estilo="none";
           $elemento =CambioAfp::find($id);
        }

      return view('thumano.EmpleadoVacaciones.formulario', array('elemento' => $elemento,"id"=>$id,"ruta"=>$ruta,"user"=>$user,"Modulos"=>$modulos,
 ...
 public function delete_EmpleadoVacacione($id)
    {
      CambioAfp::destroy($id);
      return redirect('/All_EmpleadoVacacione')->with('status', "Elemento Eliminado Correctamente");
    }
