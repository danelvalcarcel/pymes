...
class EmpleadoCumpleController extends Controller
{
 protected $nombre_modulo = "Talento Humano";

    public function All_EmpleadoCumple(Request $request)
    {
        //
        ...
        if($request["busquedad"]){
            $EmpleadoCumples = CambioAfp::where("fecha_desde",">=",$request["busquedad"])
            ->where("fecha_hasta","<=",$request["busquedad"])
            ->where('id_establecimiento', '=', $user->id_establecimiento)
            ->paginate(10);
        }else{
            $EmpleadoCumples = CambioAfp::where("id",">",0)
            ->where('id_establecimiento', '=', $user->id_establecimiento)
            ->paginate(10);
        }
         ...
         return view('thumano.EmpleadoCumples.home', array("EmpleadoCumples"=>$EmpleadoCumples,"title_menu"=>"",
            "title"=>"Cumpleaños Empleados","user"=>$user,"Modulos"=>$modulos,
            "nombre_modulo"=>$this->nombre_modulo)); 
    }



    public function EmpleadoCumple_update(Request $request){
          ...
          $elemento1->save();
         return redirect('/All_EmpleadoCumple')->with('status', "Elemento Actualizado Correctamente");
     ...
     public function EmpleadoCumple_create(Request $request){
        ...
        return redirect('/All_EmpleadoCumple')->with('status', "Elemento Creado Correctamente");
    }




    public function formulario_EmpleadoCumple($id,$ruta)
    {
        //
        ...
        if($ruta=="actualizar"){
          $ruta ="EmpleadoCumple_update";
           $elemento =CambioAfp::find($id);
           $estilo="none";
        }else if($ruta=="crear"){
          $ruta ="EmpleadoCumple_create";
          ...
          $elemento1->idtipoEmpleadoCumple ="";
          ...
          $elemento = $elemento1;
           ...
           $ruta ="All_EmpleadoCumple";
           $estilo="none";
           $elemento =CambioAfp::find($id);
        }

      return view('thumano.EmpleadoCumples.formulario', array('elemento' => $elemento,"id"=>$id,"ruta"=>$ruta,"user"=>$user,"Modulos"=>$modulos,
 ...
 public function delete_EmpleadoCumple($id)
    {
      CambioAfp::destroy($id);
      return redirect('/All_EmpleadoCumple')->with('status', "Elemento Eliminado Correctamente");
    }
